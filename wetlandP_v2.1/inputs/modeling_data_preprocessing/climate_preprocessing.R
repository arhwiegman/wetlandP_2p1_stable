...
df.climate.full <- df.climate.raw %>%
  ...
  mutate(REPORT_TYPE=case_when(
    str_detect(REPORT_TYPE...95,"^FM.*") ~ REPORT_TYPE...95,
    str_detect(REPORT_TYPE...95,"^SOD.*")~ REPORT_TYPE...95,
    str_detect(REPORT_TYPE...95,"^SOM.*")~ REPORT_TYPE...95,
    TRUE ~ REPORT_TYPE...3)) %>%
  mutate(report_dt=case_when(
    str_detect(REPORT_TYPE...95,"^SOD.*")~"day",
    str_detect(REPORT_TYPE...95,"^SOM.*")~"month",
    str_detect(REPORT_TYPE...95,"^FM.*")~"hr",
  )) %>%
...
save(df.ET.PM,file="df.ET.PM.Rdata")
...
save(df.ET.PT,file="df.ET.PT.Rdata")  
