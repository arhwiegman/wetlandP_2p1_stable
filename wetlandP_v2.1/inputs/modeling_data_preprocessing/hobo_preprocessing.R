...
# DEFINE FUNCTIONS AND CODE BLOCKS #################################
# code block "expression" to make plots 
e_make_plots <- expression({
  
  # EXAMINE RECORD of Barometric Pressure
  d %>% 
...
e_load_concat_hobo_data <- expression({
  ...
  # Update readout logs
  getwd()
  # point the path to the folder containing hobo readouts in .xlsx format
...
df.hobo.wetland_summary <- 
  df.hobo.wetland.Hw_ind %>% 
  group_by(site,plot,site_plot,lat,lon,elevation) %>% 
  select(starts_with("Hw"),starts_with("ind")) %>%
  summarize_all(list(
                 ~mean(.),
                 ...
                 P99p9 = ~quantile(.,probs=0.999)
               ),na.rm=T) %>%
  group_by(site,plot,site_plot,lat,lon,elevation) %>% 
  select(starts_with("ind")&!contains("P"),
         starts_with("Hw")) %>% 
  rename_with(
      .cols=contains("ind"),
      .fn=~str_remove(.x, "_mean"))
  
# SAVE DATA #####################
write_csv(df.hobo.info,paste0(path,"HOBO_info.csv"))
write_csv(df.hobo.data,paste0(path,"HOBO_data.csv"))
write_csv(df.hobo.joined,paste0(path,"HOBO_joined.csv"))
write_csv(df.hobo.data_processed_nested %>% unnest(data),paste0(path,"HOBO_data_processed.csv"))
save(df.hobo.wetland ,file=paste0(getwd(),"/","df.hobo.wetland.Rdata"))
save(df.hobo.wetland_ma ,file=paste0(getwd(),"/","df.hobo.wetland_ma.Rdata"))
save(df.hobo.wetland_summary ,file=paste0(getwd(),"/","df.hobo.wetland_summary.Rdata"))
