...
sub_set_parameters <- expression({
  ...
  IO_in_IM = T # logical T/F or 1/0  | toggles inflow of inorganic matter as ISS | 
  IO_in_PIP = T #logical T/F or 1/0 | toggles inflow of PIP | 
  IO_in_LOP = T # logical T/F or 1/0 | toggles inflow of labile organic P | 
  IO_in_ROP = T # logical T/F or 1/0 | toggles inflow of recalcitrant organic P | 
  IO_in_DIP = T # logical T/F or 1/0 | toggles inflow of dissolved inorganic P | 
  IO_out_IM = T # logical T/F or 1/0 | toggles outflow of IM | 
  IO_out_PIP = T # logical T/F or 1/0 | toggles outflow of PIP | 
  IO_out_LOP = T # logical T/F or 1/0 | toggles outflow of LOP | 
  IO_out_ROP = T # logical T/F or 1/0 | toggles outflow of ROP | 
  IO_out_DIP = T # logical T/F or 1/0 | toggles outflow of DIP |
  
  # process functions 
  IO_DIP_E_langmuir = F # logical T/F or 1/0 | turns on the use of langmuir model for caclulating DIP_E |
  IO_variable_k_E = T # logical T/F or 1/0 | toggles variable calculation of k_E |
  IO_variable_k_Ex_max = F # logical T/F or 1/0 | toggles on variable calculation of Ex_max using statistical fit to fines and LOI |
  IO_anoxic = F # logical T/F or 1/0 | toggles anaerobic conditions for DIP_E concentration |
  
  IO_variable_DIP_E = F # logical T/F or 1/0 | toggles variable calculation of DIP_E | if = F, then k_DIP_E is used
  IO_Q_net = T # logical T/F or 1/0 | toggles calculation of inflow and outflow from Qnet, Vw and HRT | see hydrology subroutine
  IO_HRT_power_model = F # logical T/F or 1/0 | toggles calculation HRT from a power model | if Zw > 0, HRT = a*Zw^b, where Zw is elevation relative to lowest elevation in the wetland, and b<0
  ...
  # decay
  k_f_labile = 0.8 # g/g| labile fraction organic matter | Morris & Bowden 1986 refractory fraction of 0.2 k_f_LOM_OSS = k_f_labile  # g/g| labile fraction incoming organic suspended solids| fit to field data
  ...
  # sedimentation
  k_rp_o = 4.5e-7  # m| average radius of organic particles| input
  k_rp_i = fn_particle_radius(sand=1-k_f_fines,silt=k_f_fines-k_clay,clay=k_clay)  # m| average radius of inorganic particles| based on soil texture see `fn_particle_radius`` 
  k_u_o = fn_settlingvelocity(k_rp = k_rp_o,k_dp = k_dp_o) # m/d| organic particle settling velocity| assumed constant laminar/slow flow (Reddy & Delaune 2008); see `fn_settlingvelocity`
  k_u_i = fn_settlingvelocity(k_rp = k_rp_i,k_dp = k_dp_i) # m/d| inorganic particle settling velocity| assumed laminar/slow flow (Reddy & Delaune 2008) ; see `fn_settlingvelocity`
})