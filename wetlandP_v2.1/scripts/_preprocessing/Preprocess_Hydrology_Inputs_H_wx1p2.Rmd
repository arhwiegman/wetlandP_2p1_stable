...
sub_hydrology_preprocessing <- expression({
  ...
  df.H.day.pred <- df.H.day %>% 
    ...
    mutate(
      ...
      # MULTIPLYING BY 1.2 to amplify water levels
      Hw=(Zw-Zs)*1.2,
      Hw=ifelse(Hw<0,0,Hw),
  ...
  write_csv(df.hc.unit,paste0("df.hydroclimate.1m.",sp,"x1p2.csv"))
  save(df.hc.unit,file=paste0("df.hydroclimate.1m.",sp,"x1p2.Rdata"))
}) # end subroutine
```
