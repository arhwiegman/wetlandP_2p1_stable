forcingfile = paste0("df.hydroclimate.1m.",df.sim.pars$site.plot[j],".csv")
inputdir=file.path(wrkdir,"inputs/")
# model assumptions_ site and plot (driving hydrograph)_ soil PSR _ and stream concentrations 
simname = paste0(parsheet,"_",df.sim.pars$sim_num[j],"_",df.sim.pars$site.plot[j],"_S",df.sim.pars$Sx[j],"_C",df.sim.pars$Cx[j])
