...
#   to establish range of potential P retention 
#   from in Vermont Low Energy Floodplain Wetlands for FFI 
...
sub_update_forcings_and_parameters <- fn_parse_r_script("scripts/_implementations/edit_parameters_processes_ffi_sensitivity.R")

sub_loop_parsheets_and_loop_sims <- expression({
  # parsheet must be declared before runing this loop
  try({
    df.sim.pars <- readxl::read_xlsx("inputs/lcbp_sites/ffi_input_concentrations.xlsx",
    ...
    sites <- df.sim.pars$sim_num
    rowindexes = which(df.sim.pars$sim_num %in% sites)
    # LOOP THROUGH INPUT FILES FOR EACH SITE
    for (s in sites){
      rm(list=vars)
      j <- which(df.sim.pars$sim_num == s)
      print(s)
...
sub_post_process_ffi_sensitivty <- expression({
  df.sim.pars <- readxl::read_xlsx("inputs/lcbp_sites/ffi_input_concentrations.xlsx",
                                   sheet=parsheet,skip=1) %>%
    mutate(across(starts_with("IO_"),~as.logical(.))) %>% 
    mutate(
...
sheets = readxl::excel_sheets("inputs/lcbp_sites/ffi_input_concentrations.xlsx")
parsheets = sheets[sheets %>% str_detect("pars") %>% na.omit]
#parsheet = parsheets[1]
# RUN THE MODEL -----
for (parsheet in parsheets){
  eval(sub_loop_parsheets_and_loop_sims)
}

s# POST PROCESS THE DATA ----
for (parsheet in parsheets){
  try({
    eval(sub_post_process_ffi_sensitivty)
  })
}
...
# ------------ 
df.sim.outs_summary %>% write_csv(paste0(wrkdir,"/outputs/output_summary_ffi_sensitivity","_",format(Sys.time(), "%Y-%m-%d_%h%m%s"),".csv"))
save.image(paste0(wrkdir,"/outputs/env_","xecute_ffi_sensitivity","_",format(Sys.time(), "%Y-%m-%d_%h%m%s"),".Rdata"))
# # =========END OF FILE===================
