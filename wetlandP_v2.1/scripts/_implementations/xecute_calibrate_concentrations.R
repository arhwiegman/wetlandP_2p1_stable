...
sub_loop_sites_calc_cost <- expression({
  ...
  for (s in site.sample){
    ...
    eval(sub_set_forcings_and_parameters_for_sim)
    eval(sub_initialize)
...
sub_loop_parsheets_and_loop_sims <- expression({
  ...
  try({
    ...
    sites <- df.sim.pars$site.plot[c(5,6)]
    ...
    for (s in sites){
      ...
      try(system.time({
        ...
        eval(sub_initialize)
        fn_sim_ode_accretion(make_plots=T,save_data=F)
      }))
    } # end for
  }) # end try
}) # end sub
...
eval(sub_evaluate_source_code)
sub_update_forcings_and_parameters <- fn_parse_r_script("scripts/edit_parameters_processes_concentrations.R")
...
#   print(s)
#   setwd(file.path(wrkdir,"scripts")) # navigate to scripts folder 
#   cat("sourcing 'edit_parameters_processes=.R'\n")
#   source("edit_parameters_processes_concentrations.R")
#   # NAME OUTPUT FILES -----------------------------------------------
#   setwd(wrkdir) # set wd back to working directory.
