...
eval(sub_evaluate_source_code)
sub_update_forcings_and_parameters <- fn_parse_r_script("scripts/_implementations/edit_parameters_processes_lcbp_sensitivity_stoke.R")
...
write_csv(df.sim.outs_summary,paste0("outputs/outputs_summary_lcbp_sensitivity_stoke",format(Sys.time(),"%Y-%m-%d_%Hm%Mm"),".csv"))
...
# ------------ 
save.image(paste0(wrkdir,"/outputs/env_","xecute_lcbp_sensitivity_stoke","_",format(Sys.time(), "%Y-%m-%d_%h%m%s"),".Rdata"))
# # =========END OF FILE===================
