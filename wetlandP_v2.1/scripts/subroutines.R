...
sub_init_states <- expression({ 
  ...
  LOM_a = 0 #(H_w*area)*OSS*(k_f_labile) # g d.w.;labile organic sediment mass aboveground; 
  ROM_a = 0#(H_w*area)*OSS*(1-k_f_labile) # g d.w.; recalcitrant organic sediment mass aboveground;
  IM_a = 0#(H_w*area)*ISS
  DIP_a = 0#(H_w*area)*SRP
  LOP_a = 0#ROM_a*k_BM2P*(k_f_labile)
  ROP_a = 0#ROM_a*k_BM2P*(1-k_f_labile)
  PIP_a = 0#IM_a*k_ISS2P + (H_w*area)*SRP*k_SRP2PIP
  ...
  if(IO_variable_k_Ex_max){
    AlFe_ox = fn_AlFe_ox_fines(LOI = k_LOI,
                               k_f_fines = k_f_fines)
  ...
  if(IO_DIP_E_langmuir){
    if(IO_variable_k_E){
      ...
      E = fn_k_E_statmodel(k_LOI=k_LOI,
                           k_PSR=k_PSR,
                           k_Ex_max = k_Ex_max)
    }
    # DIP calculated from power model of DIP = a(Smax - S)(1-PSR)^(-b)
    # Defined operationally as SRP = a((OxAl/27+OxFe/56) - OxP/31)/(1-OxPSR)*^(-b)
    DIP_E = fn_DIP_E_langmuir(Ex=k_PSR*Ex_max,
                              Ex_max=Ex_max,
                              E=E)
  }else{
    DIP_E = fn_DIP_E_power_law(anoxic=IO_anoxic,
                               Ex_max = Ex_max,
                               Ex = Ex)
  }
  DIP_E = ifelse(IO_variable_k_E,k_DIP_E,DIP_E)
  PIP_b = (Ex_max)*PSR*(TM_b*10^-3)
...
sub_init_extended <- expression({
  ...
  outgroups <- list(states=states,derivs=derivs,rates=rates,vars_mass=vars_mass,vars_elev=vars_elev,vars_pore=vars_pore,vars_hydro=vars_hydro,vars_adsorp)
  if(floor((t %% 1)*100)==0){
...
sub_intermediates <- expression({
  ...
  # oxygen status
  #IO_anoxic = HRT>7&((TW<1)|(TW>25))
  
  # adsorption 
  eval(sub_adsorption_intermediates)
  
})

sub_adsorption_intermediates <- expression({
  # P Saturation Ratio
  Ex = PIP_b/(TM_b*1e-3)
  PSR = Ex/Ex_max
  # adsorption rate of DIP to PIP
  if(IO_variable_k_Ex_max){
    AlFe_ox = fn_AlFe_ox_fines(LOI = LOI_b,
                               k_f_fines = k_f_fines)
    # Al + Fe (mmol/kg) increases as a function of fine sediment and LOI 
    # Updated AlFe ~ LOI*(1-k_f_fines) relationship based on data from Ch1 and perillo
    Ex_max = 31*AlFe_ox/1000}
  ...
  if(IO_DIP_E_langmuir){
    if(IO_variable_k_E){
      # DIP calculated from Langmuir model (Wang et al. 2003)
      # Langmuir bond energy modeled based on statistical fit of soil data to k_E estimates of intact cores across 20 sample sites
      E = fn_k_E_statmodel(k_LOI=LOI_b,
                           k_PSR=PSR,
                           k_Ex_max = Ex_max,
                           treatmentO2 = !IO_anoxic)
    }
    # DIP calculated from power model of DIP = a(Smax - S)(1-PSR)^(-b)
    # Defined operationally as SRP = a((OxAl/27+OxFe/56) - OxP/31)/(1-OxPSR)*^(-b)
    DIP_E = fn_DIP_E_langmuir(Ex=Ex,
                              Ex_max=Ex_max,
                              E=E)
  }else{
    DIP_E = fn_DIP_E_power_law(anoxic=IO_anoxic,
  ...
  if(IO_variable_DIP_E){
    DIP_E = ifelse(IO_anoxic,k_DIP_E_N2,k_DIP_E_O2)
  }
})
...
sub_diffeqs <- expression({
  ...
  #d_ROP_a = -1*sed_ROP + 1*dec_litter2ROP_a + -1*dec_ROP_a + 1*in_ROP + -1*out_ROP
  #d_LOP_a = -1*sed_LOP + 1*dec_litter2LOP_a + -1*dec_LOP_a + 1*dec_ROP_a + 1*in_LOP + -1*out_LOP
  d_ROP_a = -1*sed_ROP + -1*dec_ROP_a + 1*in_ROP + -1*out_ROP
  d_LOP_a = -1*sed_LOP + -1*dec_LOP_a + 1*dec_ROP_a + 1*in_LOP + -1*out_LOP
  d_PIP_a = -1*sed_PIP + 1*in_PIP + -1*out_PIP
  d_DIP_a = 1*dec_LOP_a + 1*diff_DIP_b2a + 1*in_DIP + -1*out_DIP
  # d_ROP_b = 1*mort_rootP2ROP + 1*sed_ROP + -1*dec_ROP_b
  # d_LOP_b = 1*mort_rootP2LOP + 1*sed_LOP + -1*dec_LOP_b + 1*dec_ROP_b
  d_ROP_b = 1*mort_rootP2ROP + 1*sed_ROP + -1*dec_ROP_b + 1*dec_litter2ROP_a 
  d_LOP_b = 1*mort_rootP2LOP + 1*sed_LOP + -1*dec_LOP_b + 1*dec_ROP_b + 1*dec_litter2LOP_a 
  d_PIP_b = 1*sed_PIP + 1*sorp_DIP2PIP_b
  d_DIP_b = 1*dec_LOP_b + -1*diff_DIP_b2a + -1*sorp_DIP2PIP_b + -1*assim_shootP +-1*assim_rootP
})
...
sub_compile_extended <- expression({
  ...
  vars_adsorp <- c(E=k_E,Ex_max=k_Ex_max,PSR=PSR,DIP_E=DIP_E)
